...
The variables selected by lasso give a global sense of the more influential predictors in driving the model, however if the data is complex, in some regions of the data space the variables explaning the classifications could be completely different to the ones selected by the surrogate model.  
...
plot_explanations(explanation_PCA)
```
...
print(coeffs_pca_sur[,colSums(coeffs_pca_sur) != 0][,1:5])
```

```{r}
# crea el sim data y predice sus outputs con predict_model
#case_res <- predict_model(x = black_box, newdata = as.data.frame(x_train), type = 'raw') %>% tibble::as_tibble()    #case_res <- set_labels(case_res, black_box)

# selecciona los n_features mas importantes con lasso teniendo en cuenta los pesos
#labels <- names(case_res)[order(as.data.frame(case_res)[1,], decreasing = TRUE)[seq_len(10)]]
#fit <- glmnet(x[shuffle_order,], case_res[[labels[label_i]]], weights = weights[shuffle_order], alpha = 1, nlambda = 300)

# fits with ridge model
#fit <- glmnet(x[shuffle_order, features], y[[label]][shuffle_order], weights = weights[shuffle_order], alpha = 0, lambda = 2 / length(y[[label]]))

####################

case_res <- predict_model(x = black_box, newdata = as.data.frame(x_train), type = 'raw') %>% tibble::as_tibble()

get_sur_cof <- function(cat, n_features=10)
{
  feat_selection <- glmnet(x_train_pca$x[,1:pcs_n], case_res[[cat]], alpha = 1, nlambda = 300)
  ...
  glmnet(x_train_pca$x[,features], case_res[[cat]], alpha = 0, lambda = 2 / length(case_res[[cat]]))
}

fit = get_sur_cof("LUAD")
r2 <- fit$dev.ratio
aacoefs <- coef(fit)
```
...
```{r message=FALSE, warning=FALSE, include=FALSE}
get_category_coeffs <- function(category, n_permutations = 4000)
{
...
```{r echo=FALSE}
print(coeffs_LUAD[1:10,colSums(coeffs_LUAD) != 0][,1:5])
print(coeffs_PRAD[1:10,colSums(coeffs_PRAD) != 0][,1:5])
```
...
round(apply(coeffs_PRAD[,names(infl_comp_PRAD)[1:3]], 2, sd),5)

cat("\n\nstandard deviation of first 3 more influential component coefficients for LUAD:\n")
round(apply(coeffs_LUAD[,names(infl_comp_LUAD)[1:3]], 2, sd),5)
```
...
```{r echo=FALSE, fig.height=17, fig.width=6, message=FALSE, warning=FALSE}
fun = median

coeffs_pca = t(as.matrix(coeffs_pca_sur))
coeffs_pca = cbind(coeffs_pca,apply(coeffs_BRCA, 2, fun))
...
coeffs_pca = coeffs_pca[,c(1,6,2,7,3,8,4,9,5,10)]
colnames(coeffs_pca) = c("BRCA (global)", "BRCA (lime)", "COAD (global)", "COAD (lime)", "KIRC (global)", "KIRC (lime)", "LUAD  (global)", "LUAD (lime)", "PRAD (global)", "PRAD (lime)")
...
heatmap.2(coeffs_pca, col= my_palette, breaks=col_breaks, symkey=FALSE, key=TRUE, Rowv=NA, Colv=NA,
          offsetCol = -104, srtCol=45, cexCol=1.2, adjCol = c(0,1))
```
